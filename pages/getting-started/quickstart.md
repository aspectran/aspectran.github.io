...
1. 요청 URI가 `/example/simplest/helloWorld`인 요청이 들어옵니다.
2. 요청 URI와 맵핑된 Translet이 요청을 건네받아서 관련 Aspect를 찾습니다.
3. Translet의 작업이 시작되기 전에 `simplestAdvice` Bean의 `wellcome` Method가 먼저 실행됩니다.
4. Translet 내부의 `simplestAction` Bean의 `helloWorld` Method를 실행합니다.
5. Translet의 작업이 완료된 후에 `simplestAdvice` Bean의 `goodbye` Method가 실행됩니다.
6. Translet은 모든 실행 결과 값을 집계하여 `plain/text` 형식의 컨텐츠로 출력을 하고 작업을 종료합니다.
