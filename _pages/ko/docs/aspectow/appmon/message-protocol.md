...
본 프로토콜은 가독성이 높은 텍스트 기반 구조를 채택하여 디버깅이 용이하면서도, 서브타입을 통한 유연한 확장성을 제공합니다.
{% endcapture %}
...
| 필드명 | 설명 | 비고 |
| :--- | :--- | :--- |
| **nodeId** | 데이터를 생성한 물리/논리 서버 노드의 식별자 | 예: `node01`, `server1` |
...
| **exporterName** | 해당 타입 내에서의 고유 리소스 이름 | 예: `activity`, `session`, `root.log` |
...
*   **실시간 로그 (LOG)**: 별도의 서브타입이 없습니다.
    *   형식: `node1:log:root.log:로그내용...`
*   **과거 로그 (PREVIOUS)**: `p` 서브타입을 사용합니다.
    *   형식: `node1:log/p:root.log:로그내용...`
...
*   **차트 데이터 (CHART)**: `chart` 서브타입을 사용하며 본문은 JSON입니다.
    *   형식: `node1:data/chart:activity:{"labels":[...], "data1":[...], "rolledUp":false}`
    *   기존의 래핑 레이어를 제거하고 평탄화된 JSON 구조를 제공합니다.

### 2.3 이벤트 (Event)
시스템 상태 변경이나 사용자 세션 이벤트를 전송합니다. 본문은 JSON입니다.

*   형식: `node1:event:session:{"numberOfActives":10, "createdSessions":[...], ...}`

### 2.4 메트릭 (Metric)
단일 지표 값을 전송합니다. 본문은 JSON입니다.

*   형식: `node1:metric:jvm.memory:{"format":"{used}/{max}", "data":{"used":"512MB", "max":"2GB"}}`
...
| :--- | :--- | :--- |
| **refresh** | 차트나 메트릭 등의 최신 데이터를 명시적으로 요청 | `node`, `app`, `dateUnit`, `dateOffset` |
| **loadPrevious** | 로그 박스 상단에서 이전 로그 데이터를 요청 | `node`, `app`, `logName`, `loadedLines` |
| **join** | 실시간 모니터링 세션을 시작하고 노드/앱 그룹에 참여 | `appsToJoin`, `timeZone` |

### 3.3 명령 예시

*   **로그 박스 이전 로그 요청**:
    `command:loadPrevious;node:node1;app:appmon;logName:root.log;loadedLines:500`
    *(설명: node1 노드의 appmon 애플리케이션에 대한 root.log에서 이미 로딩된 500라인 이전의 데이터를 요청함)*

*   **특정 노드 및 애플리케이션 데이터 리프레시**:
    `command:refresh;node:node1;app:jpetstore;dateUnit:hour`
    *(설명: node1 노드의 jpetstore 애플리케이션 데이터를 시간(hour) 단위로 다시 집계하여 요청함)*

*   **모니터링 세션 참여**:
    `command:join;appsToJoin:node1/appmon,node2/jpetstore;timeZone:Asia/Seoul`
    *(설명: 지정된 노드 및 애플리케이션 모니터링 그룹에 조인하며 클라이언트의 타임존 정보를 전달함)*
