...
teaser: "Aspectow AppMon의 3계층(Group - Node - App) 데이터 모델과 사전 집계(Pre-aggregation) 아키텍처를 상세히 설명합니다."
subheadline: "Aspectow AppMon"
---

Aspectow AppMon은 대규모 분산 환경에서 발생하는 애플리케이션 지표를 실시간으로 모니터링하고, 방대한 데이터를 효율적으로 집계 및 시각화하기 위해 설계되었습니다. 이 문서는 AppMon의 핵심 데이터 모델과 성능 최적화를 위한 사전 집계(Pre-aggregation) 아키텍처를 상세히 설명합니다.

## 1. 핵심 메타데이터 컬럼

이벤트 카운트 테이블은 `node_id`, `app_id`, `event_id`, `datetime`으로 구성된 복합 기본 키(PK)와 그룹 단위 집계를 위한 `group_id` 인덱스를 사용하여 데이터를 관리에 최적화된 형태로 식별합니다.

| 컬럼 | 데이터 타입 | 설명 |
| :--- | :--- | :--- |
| **`node_id`** | `varchar(30)` | **서버 노드(Server Node)**의 식별자입니다. 클러스터링된 환경에서 각 물리적/논리적 서버를 구분합니다 (예: `appmon-node1`, `node01`). |
| **`group_id`** | `varchar(30)` | **노드 그룹(Node Group)**의 식별자입니다. 여러 노드를 논리적인 서버 그룹으로 묶어 통합 관제할 수 있게 합니다 (예: `group1`, `backend-api`). |
| **`app_id`** | `varchar(30)` | 서버 내에서 실행되는 **애플리케이션**의 식별자입니다. 컨텍스트 경로와 동일한 명칭을 주로 사용합니다 (예: `jpetstore`, `petclinic`, `demo`). |
| **`event_id`** | `varchar(30)` | 추적 중인 특정 지표 또는 활동의 식별자입니다 (예: `activity`, `session`). |
...
1.  **원본 계층 (`appmon_event_count`)**
    ...
    *   인덱스: `group_id`, `app_id`, `event_id`, `datetime` 결합 인덱스를 통해 그룹 단위 차트 조회를 고속화합니다.

2.  **시간 요약 계층 (`appmon_event_count_hourly`)**
    *   해상도: 1시간 단위 (원본 대비 데이터양 약 1/12)
    *   집계 논리: `total`은 해당 시간의 마지막 누적값, `delta/error`는 해당 시간 내 모든 변화량의 합계(`SUM`).
    *   용도: 최근 며칠간의 시간대별 트렌드 분석 (**Hour View**).

3.  **최종 상태 관리 (`appmon_event_count_last`)**
    *   용도: 각 노드/애플리케이션/이벤트별 가장 최근 수집된 지표를 관리하여 증분 업데이트 및 복원을 지원합니다.
...
| **Day/Month/Year View** | `appmon_event_count_hourly` | 시간 단위 데이터를 일/월/년으로 묶음. 처리 대상 로우가 **원본 대비 8.3% 수준**으로 감소 |

### 집계 데이터의 정합성 유지
데이터가 수집될 때마다 `ON DUPLICATE KEY UPDATE` (또는 `MERGE`) 구문을 사용하여 요약 테이블의 해당 시간 슬롯에 `delta`와 `error` 값을 지속적으로 합산(Accumulate)합니다. 이를 통해 별도의 배치 작업을 기다릴 필요 없이 언제나 최신화된 집계 데이터를 대시보드에 즉시 반영할 수 있습니다.

## 5. 데이터베이스 스키마 스크립트

각 DB 플랫폼별 공식 스키마 스크립트는 다음 경로에서 확인할 수 있습니다.
