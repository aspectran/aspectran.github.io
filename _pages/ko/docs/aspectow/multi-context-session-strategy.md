...
Aspectran 세션 매니저에 대한 보다 상세한 아키텍처와 핵심 컴포넌트 설명은 [Aspectran Session Manager: Deep Dive](https://aspectran.com/ko/docs/guides/aspectran-session-manager/) 문서를 참조하십시오.
...
여러 컨텍스트가 **동일한 `ServletSessionConfig`(쿠키 설정)를 공유**하지만, `SessionManager`는 각자 가집니다.
...
*   **추적성**: L7 로드밸런서의 Sticky Session을 위해 `routeId`가 설정된 경우, 세션 ID 접미사(`.routeId`)를 통해 어느 노드/경로에서 처리되는지 식별할 수 있습니다.
...
### ⚠️ `routeId` 설정과 L7 로드밸런서 연동 (권장)
`SessionIdGenerator`에 `routeId`가 지정되면 생성되는 세션 ID 뒤에 접미사(`.routeId`)가 포함됩니다. 다중 WAS 노드로 클러스터링된 환경에서 L7 로드밸런서(HAProxy, Nginx, ALB 등)와 결합할 때 다음과 같은 이점이 있습니다.
*   **L7 Sticky Session 라우팅**: 로드밸런서는 세션 ID의 접미사를 파싱하여 해당 세션을 생성/보유한 동일 노드로 후속 요청을 안정적으로 라우팅할 수 있습니다.
*   **설정 제약**: `routeId`에는 마침표(`.`)를 포함할 수 없습니다. 설정하지 않으면(`null`) 접미사 없이 `SecureRandom` 기반의 난수와 카운터로 조합된 순수 세션 ID가 발급됩니다.
...
Aspectow의 세션 설정은 유연하지만, **"하나의 디렉토리는 하나의 SessionManager 인스턴스만 관리해야 한다"**는 원칙을 반드시 지켜야 합니다. 세션 공유가 필요한 경우 ID만 공유할지(패턴 2), 아니면 단일 매니저 공유를 통해 데이터까지 공유할지(패턴 3)를 설계 단계에서 명확히 결정하십시오.
