...
teaser: Aspectow Console 및 노드 관리자(Node Manager)의 클러스터 통신 모드, APON 설정 파일 명세, DB 및 Redis 환경 구성, Aspectran 빈 설정 방법을 다루는 종합 구성 안내서입니다.
...
Console의 환경 구성 파일은 크게 노드 및 클러스터 제어 설정(`node-config.apon`), 데이터베이스 접속 구성(`aspectow-console.db-*.properties`), Redis 클러스터링 설정(`redis.properties`), AppMon 모니터링 설정(`appmon-config.apon`), 그리고 Aspectran 빈 정의 파일(`node-rules.xml`)로 분리됩니다.
...
*   **`appmon-config.apon`**: 통합 탑재된 AppMon 모니터링 엔진의 데이터 수집기 및 집계 주기 설정 파일
...
## 3. 클러스터 통신 모드 선택 가이드 (Direct vs Gateway)
...
## 4. Redis 클러스터링 설정 (`redis.properties`)
...
## 5. Console 데이터베이스 구성 (`aspectow-console.db-*.properties`)

Aspectow Console은 관리자 계정, 역할 권한, Vault 보안 토큰, 모니터링 사전 집계 통계를 DB에 영구 보관합니다. 사용 환경에 맞는 DB 프로퍼티 파일(H2, MariaDB, MySQL, Oracle, Supabase)을 수정하여 접속 정보를 지정합니다.

### MariaDB 구성 예시 (`aspectow-console.db-mariadb.properties`)
...
Java 시스템 프로퍼티로 활성화할 DB 프로필을 전달합니다:
`-Daspectran.profiles.base.console=mariadb`

## 6. Aspectran XML 빈 구성 (`node-rules.xml`)
