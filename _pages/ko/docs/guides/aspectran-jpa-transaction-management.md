...
JPA 표준 `EntityManager`는 강력하지만, 복잡한 동적 쿼리를 작성할 때 문자열 기반의 JPQL을 사용해야 하는 제약이 있습니다. Aspectran은 이러한 불편을 해소하기 위해 **QueryDSL**을 엔진 레벨에서 통합하여 제공합니다.
...
트랜잭션의 동작 방식을 코드 레벨에서 세밀하게 제어해야 할 경우 사용하는 방법입니다.
...
public class ConsoleTxAspect extends EntityManagerAdvice {
    ...
    @Before
    public void before() {
        // 트랜잭션 활성화를 위한 필수 메서드 정의
        // super.open() 호출 여부에 따라 지연 오픈 또는 즉시 오픈을 결정할 수 있습니다.
    }
