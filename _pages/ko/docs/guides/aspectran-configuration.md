...
- **`properties`**: 여기에 정의된 키-값 쌍은 시스템 속성(`System.setProperty()`)으로 등록됩니다.

#### 시스템 속성 설정 우선순위 및 암호화 설정 리로드

Aspectran은 시스템 속성을 설정할 때 다음과 같은 원칙을 따릅니다:

1.  **커맨드라인 우선순위 존중**: Java 실행 시 `-D` 옵션으로 이미 설정된 속성이 있다면, `aspectran-config.apon`에 정의된 값보다 커맨드라인 값이 우선적으로 유지됩니다.
2.  **루트 서비스 선점 (First-started Wins)**: 여러 서비스(예: DaemonService, WebService)가 순차적으로 구동될 때, 가장 먼저 실행되는 서비스(일반적으로 루트 서비스)가 설정한 값이 시스템 전역에 고착됩니다. 이후에 실행되는 서비스들은 이미 설정된 값을 덮어쓸 수 없습니다.
3.  **지능적 암호화 설정 리로드**: `aspectran.encryption.password` 등 암호화 관련 속성이 시스템에 새로 주입되는 시점에 `PBEncryptionUtils.reload()`가 자동으로 호출됩니다. 이를 통해 프레임워크 전반에서 사용되는 암호화 유틸리티가 최신 설정값으로 즉시 동기화됩니다.

> **참고:** 하나의 JVM 프로세스 내에서는 모든 서비스가 동일한 암호화 설정을 공유하는 것이 권장됩니다. 만약 서비스별로 다른 설정을 구성하더라도, 시스템은 가장 먼저 구동된 서비스의 설정을 기준으로 동작하게 됩니다.
...
| `evaluate` | `com.aspectran.shell.command.builtins.EvaluateCommand`<br/>AsEL(Aspectran Expression Language) 표현식을 평가합니다. |
