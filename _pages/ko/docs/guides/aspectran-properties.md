...
### 2.3. 경로(Path) 및 로그(Logging) 설정
애플리케이션의 작업 디렉토리와 로그 저장 위치를 제어합니다. 지정된 경로가 존재하지 않을 경우 Aspectran이 **자동으로 디렉토리를 생성(`mkdirs()`)**합니다.
*   `aspectran.basePath`: 애플리케이션의 루트 디렉토리입니다.
*   `aspectran.workPath`: 애플리케이션의 작업 디렉토리입니다. (기본값: `basePath/work`)
*   `aspectran.tempPath`: 임시 파일 저장 디렉토리입니다. (기본값: `basePath/temp`)
*   `aspectran.logsDir`: 로그 파일 저장 디렉토리입니다. (기본값: `basePath/logs`)
*   `aspectran.archivedLogsDir`: 보관된(Archived) 로그 저장 디렉토리입니다.
*   `aspectran.logCharset`: 로그 기록 시 사용할 문자셋입니다. (예: `UTF-8`)

### 2.4. 프레임워크 기능 설정
*   `aspectran.encryption.algorithm`: 설정 파일 내 암호화된 값을 복호화할 때 사용하는 알고리즘입니다. (기본값: `PBEWITHHMACSHA256ANDAES_128`)
