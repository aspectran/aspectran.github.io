...
3.  **실행 스킵**: 락 획득에 실패한 나머지 노드들은 해당 주기에서 작업을 자동으로 건너뜁니다.
4.  **안전성 보장**: 작업 도중 노드가 비정상 종료되더라도, 락에 설정된 만료 시간(Expiration)이 지나면 자동으로 해제되어 다음 주기에서 다른 노드가 작업을 이어받을 수 있습니다.

### `isolated` 속성을 통한 실행 제어

스케줄링 규칙에는 해당 작업이 분산 대상인지, 아니면 각 노드마다 독립적으로 항상 실행되어야 하는지를 결정하는 `isolated` 속성이 있습니다.

-   **`isolated="false"` (기본값)**: 클러스터 전역에서 **단 하나의 노드**만 작업을 수행합니다. (분산 락 적용 대상)
-   **`isolated="true"`**: 클러스터 상태와 관계없이 **모든 노드**가 각자 정해진 시간에 작업을 수행합니다. 시스템 로그 수집이나 로컬 캐시 갱신과 같은 작업에 적합합니다.

> **분산 락 적용 시 트리거 유형별 제약사항 (중요)**
>
> 클러스터 환경에서 분산 락(`isolated="false"`)은 모든 노드가 정확히 동일한 절대 시각에 작업을 트리거하는 **`cron` 트리거**에서만 안정적으로 동작합니다. `cron` 트리거는 달력 기반 절대 시각(예: 매 정각 0초)을 기준으로 모든 노드의 실행 예정 시각(`scheduledFireTime`)이 일치하여 동일한 락 키로 경쟁할 수 있습니다.
>
> 반면, **`simple` 트리거**(간격 기반 반복)는 노드마다 기동 시점(부팅 시간)의 차이 및 타임 드리프트(Time Drift)로 인해 각 노드의 실행 예정 시각이 서로 다르게 계산됩니다. 따라서 동일한 락 키를 형성할 수 없어 클러스터 분산 락을 신뢰성 있게 적용할 수 없습니다.
>
> 이러한 이유로 클러스터 분산 락(`RedisScheduledJobLockProvider`)이 활성화되면, **`simple` 트리거로 설정된 모든 스케줄은 프레임워크에 의해 자동으로 `isolated="true"`(격리 모드)로 강제 전환**되어 각 노드에서 개별 실행됩니다. 따라서 클러스터 전역에서 단 하나의 노드만 실행해야 하는 작업은 반드시 **`cron` 트리거**를 사용해야 합니다.
...
Aspectran Scheduler는 프레임워크의 핵심 사상인 **추상화와 모듈화**를 잘 보여주는 기능입니다. 개발자는 Quartz API의 복잡함을 신경 쓸 필요 없이, 자신이 가장 잘 아는 **Translet**을 사용하여 원하는 작업을 손쉽게 자동화하고, **XML/APON 기반의 명시적 설정**과 **어노테이션 기반의 편리한 설정** 중 프로젝트에 가장 적합한 방식을 선택하여 스케줄러를 애플리케이션의 일부로 완벽하게 통합하고 관리할 수 있습니다. 특히 Redis 기반의 분산 락 기능은 대규모 클러스터 환경에서도 데이터 무결성과 시스템 안정성을 완벽하게 보장합니다.
