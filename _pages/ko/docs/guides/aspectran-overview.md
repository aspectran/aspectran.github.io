...
특히, **환경 추상화를 위한 어댑터 패턴**은 Aspectran의 유연성과 개발 생산성을 높이는 핵심적인 강점입니다. 어댑터는 런타임 환경(웹, 콘솔, 데몬 등)의 차이를 추상화하여, 동일한 비즈니스 로직이 어떤 환경에서든 수정 없이 실행될 수 있도록 돕습니다.
...
*   **Activity Layer**: 모든 외부 요청(웹, 셸 등)을 수신하고, 해당 요청을 처리하기 위한 Activity 컨텍스트를 생성합니다. 이 계층은 요청의 생명주기를 관리하고, 적절한 Translet을 찾아 정해진 처리 규칙에 따라 요청을 처리합니다. `Activity`는 단일 요청 처리를 위한 중앙 실행 객체이며, `ThreadLocal`을 사용하여 멀티스레드 환경에서도 안정적인 요청 처리를 보장합니다.
*   **Translet Layer**: `Activity`가 요청을 처리할 때 참조하는 `Translet` 규칙이 정의된 계층입니다. `Activity`는 이 계층에 정의된 `Translet`의 규칙에 따라 `Action`을 실행하고, `Bean`을 사용하며, 최종적으로 `View`를 통해 응답을 생성하는 흐름을 제어합니다. Translet은 요청에 대한 "설계도"이자 엔진과 사용자 코드 사이의 "인터페이스" 역할을 합니다.
*   **Bean Container Layer**: IoC/DI를 담당하는 핵심 계층으로, 애플리케이션의 모든 빈(객체)을 관리합니다. 빈의 생성, 초기화, 소멸 및 의존성 주입을 처리하여 컴포넌트 간의 느슨한 결합을 보장합니다. 싱글톤, 프로토타입 외에도 요청(Request)이나 세션(Session) 스코프를 지원하여 객체의 생명주기를 세밀하게 관리합니다.
*   **AOP Layer**: Aspectran의 AOP 엔진이 위치하는 계층입니다. Translet 실행 전후, Bean 메서드 호출 전후 등 특정 조인 포인트에 Aspect를 적용하여 횡단 관심사를 처리합니다. 런타임 동적 프록시를 통해 비즈니스 로직과 독립적으로 동작하며, `@Advisable` 등을 사용한 지능적인 최적화로 성능을 극대화합니다.
...
*   **Activity (액티비티)**: Aspectran에서 모든 요청을 처리하는 **실행 엔진**이자 요청의 생명주기를 관리하는 컨텍스트입니다. 웹 요청, 셸 명령 등 모든 종류의 요청을 일관된 방식으로 처리합니다. 모든 실행 흐름의 중심에 있으며, `ActivityContext`라는 중앙 컨테이너가 이들의 초기화와 설정을 조율합니다.
...
*   **Action (액션)**: `Activity` 내에서 실행되는 개별적인 작업 단위로, 그 실행 규칙은 `Translet`에 정의되어 있습니다. 데이터베이스 조회, 외부 API 호출, 비즈니스 로직 처리 등 특정 기능을 수행합니다. `InvokeAction`, `AnnotatedAction`, `IncludeAction` 등 다양한 유형이 있으며, 이를 계층적으로 구성하여 복잡한 비즈니스 로직을 형성합니다.
...
*   **세션 관리**: Aspectran은 웹, 셸, 데몬 등 다양한 실행 환경에서 일관된 방식으로 상태를 유지할 수 있도록 **환경에 비종속적인 고성능 세션 관리 컴포넌트**를 제공합니다. 
    *   **교체 가능한 저장소**: 파일 시스템이나 Redis 기반의 고성능 Lettuce 클라이언트를 사용한 세션 저장소 구현을 설정에 따라 선택할 수 있으며, 클러스터링 지원을 통해 엔터프라이즈급 기능을 갖추고 있습니다.
    *   **정교한 정책**: 봇 방지를 위한 신규 세션 전용 타임아웃(`maxIdleSecondsForNew`) 등 지능적인 세션 생명주기 관리 정책을 포함합니다.
*   **힌트 메커니즘 (Hint Mechanism)**: 비즈니스 로직 실행 시 해당 메서드의 실행 환경이나 처리에 필요한 부가적인 정보를 프레임워크나 다른 모듈에 전달하는 매우 유연하고 강력한 도구입니다. 
    *   `@Hint` 어노테이션을 사용하여 메서드에 "메모"를 남기듯이 선언하며, 정보는 APON 형식을 사용합니다. 
    *   이를 통해 서비스 코드가 하위 모듈의 구체적인 설정을 직접 참조하지 않고도 캐시 적용, 레이아웃 제어, 보안 설정 등을 동적으로 지시할 수 있어 **느슨한 결합(Loose Coupling)**을 유지할 수 있습니다. 
    *   또한, 힌트의 전파(Propagation)와 격리(Isolation)를 지원하여 중첩된 메서드 호출 환경에서도 정교한 제어가 가능합니다.
*   **프로필 및 환경 제어 (Profiles & Environment Management)**: 애플리케이션 설정을 특정 실행 환경(dev, test, prod 등)에 따라 동적으로 활성화하거나 비활성화할 수 있는 강력한 기능을 제공합니다. 
    *   `aspectran.profiles.active` 등의 시스템 프로퍼티를 통해 프로필을 활성화하며, 논리 연산자(`!`, `()`, `[]`)를 사용한 복잡한 활성화 조건 표현식을 지원합니다. 
    *   XML 설정의 `<properties>`, `<append>` 요소나 자바 코드의 `@Profile` 어노테이션을 통해 환경별로 다른 데이터베이스 연결 정보나 빈 구성을 손쉽게 적용할 수 있습니다.
*   **환경 설정 및 로딩**: Aspectran은 APON(Aspectran Object Notation) 또는 XML 형식의 설정 파일을 통해 애플리케이션을 구성합니다. 
    *   **고도화된 `nodelet` 파싱 엔진**을 사용하여 설정 파일을 매우 효율적으로 로딩하며, 중첩된 규칙을 동적으로 처리하는 `mount` 기능 등으로 파싱 속도를 극대화합니다.
    *   동적 리로딩 기능을 지원하여 개발 편의성을 높입니다.
*   **동적 핫 리로딩 (Hot-Reloading)**: `SiblingClassLoader`라는 독자적인 클래스로더 메커니즘을 통해 JVM 재시작 없이도 설정이나 클래스 수정을 즉시 반영할 수 있습니다. "형제 우선(Sibling-First)" 위임 모델을 사용하여 모듈 간 클래스 공유 및 동적 교체를 유연하게 지원합니다.
...
*   **로깅**: Aspectran은 SLF4J와 Logback을 기반으로 유연하고 강력한 로깅 시스템을 구축합니다. **`LoggingGroupDiscriminator`**라는 독자적인 기능을 통해 서비스나 URI 경로별로 로그를 별도의 파일로 분리하여 관리할 수 있는 강력한 기능을 제공합니다.
*   **작업 스케줄링 (Job Scheduling)**: 애플리케이션 내의 특정 작업을 정해진 시간이나 주기에 따라 자동 실행할 수 있도록 **Quartz 기반의 강력한 스케줄링 프레임워크를 내장**하고 있습니다. 
    *   별도의 라이브러리 통합 없이 설정만으로 즉시 사용 가능하며, 실행할 작업 본체로 Aspectran의 핵심 컴포넌트인 **Translet**을 그대로 활용할 수 있다는 점이 큰 특징입니다. 
    *   `simple` 트리거(간격 반복)와 `cron` 트리거(시간표 기반)를 모두 지원하며, XML 기반의 명시적 설정과 `@Schedule` 어노테이션 기반의 편리한 설정을 모두 제공합니다. 
    *   `ActivityJobReporter`를 통해 잡의 시작, 성공, 실패 이벤트를 상세히 로깅하여 모니터링할 수 있습니다.
*   **RESTful 웹 서비스**: Aspectran은 RESTful API 개발에 최적화된 기능을 제공합니다. 간결한 설정으로 HTTP 메서드(GET, POST, PUT, DELETE 등)에 따른 요청을 처리하고, JSON/XML/APON 등 다양한 형식으로 응답을 생성할 수 있습니다. 
    *   가장 구체적인 규칙을 우선하는 **지능적인 검색 우선순위**를 적용하여, `/users/*`보다 `/users/info`를 먼저 찾는 등의 합리적인 요청 매핑을 제공합니다.
...
*   **`aspectran-test`**: Aspectran 애플리케이션의 단위 테스트 및 통합 테스트를 지원하는 모듈입니다. JUnit 5와 통합되어 Aspectran 컨텍스트 내에서 빈(Bean)이나 Translet을 손쉽게 테스트할 수 있는 환경을 제공합니다.
...
개발자는 프로젝트의 성격에 따라 필요한 모듈을 조합하여 최적의 환경을 구성할 수 있습니다 (예: `core` + `web` + `with-undertow`를 통한 고성능 마이크로서비스 구축).
...
*   **임베디드 웹 서버 (Undertow, Jetty)**: 별도의 웹 서버 설치 없이 Aspectran 애플리케이션 자체에 웹 서버를 내장하여 독립적으로 실행할 수 있습니다. Undertow와 같은 고성능 네이티브 엔진을 직접 활용하면 서블릿 컨테이너의 오버헤드 없이 더욱 가볍고 빠른 웹 서비스를 구축할 수 있습니다.
...
Aspectran은 [Apache 2.0 라이선스](https://www.apache.org/licenses/LICENSE-2.0) 하에 배포됩니다. 이는 상업적 및 비상업적 목적으로 자유롭게 사용, 수정, 배포할 수 있음을 의미합니다.
