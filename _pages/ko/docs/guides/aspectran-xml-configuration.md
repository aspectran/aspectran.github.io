...
4.  **지연 파싱 메커니즘과의 완벽한 통합**:
    대상 리소스가 보류(Pending) 목록에 등록될 때 내부 자식 오버라이드 규칙도 `<append>` 정의와 함께 안전하게 보관되며, 추후 지연 파싱 시점에 대상 모듈의 구성 규칙이 모두 해석된 직후 해당 모듈 스코프를 기준으로 엄격한 검증을 거쳐 투명하게 적용됩니다.
...
-   `dependsOn`: 현재 빈이 생성되기 전에 먼저 생성되어야 하는 빈들의 ID를 지정합니다. 쉼표로 구분하여 여러 개를 지정할 수 있습니다. 이를 통해 초기화 순서와 파괴 순서(역순)를 제어할 수 있습니다.
