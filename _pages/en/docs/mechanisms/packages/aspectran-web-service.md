...
### `DefaultServletWebService` (Implementation Class)

The final implementation of `WebService`, responsible for receiving HTTP requests and handling the entire process of creating and executing `ServletWebActivity`.
...
-   `service(HttpServletRequest request, HttpServletResponse response)`: The heart of this class. All requests from the servlet container come through this method.
    ...
    4.  If it's a target for processing, it creates a `ServletWebActivity` instance. This `ServletWebActivity` acts as an adapter wrapping the `request` and `response` objects.
...
A factory class that creates and configures `DefaultServletWebService` instances.
...
-   **`com.aspectran.web.activity`**: `DefaultServletWebService` creates a `ServletWebActivity` for every valid request. `ServletWebActivity` is the most important collaborating object, acting as an adapter between the Servlet API and the core engine.
-   **`javax.servlet`**: This package is responsible for direct integration with the Servlet API. It receives `HttpServletRequest` to create `ServletWebActivity` and outputs the result to `HttpServletResponse`.
