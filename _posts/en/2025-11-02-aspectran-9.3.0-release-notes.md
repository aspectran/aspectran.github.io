...
*   **Fine-grained Scope Control for `FactoryBean`**
    *   The `FactoryBean` interface now includes an `isSingleton()` method. This allows a factory to have fine-grained control over whether the object it produces is a shared singleton (default) or a new prototype instance for each request.
